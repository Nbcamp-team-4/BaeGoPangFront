...
import { G, PRIMARY, Icon, Chip, Img, Stars, Badge, Section, Phone, HomeTopBar, TopContent } from "../components/UI";
...
function Home({go}){
  ...
  return <Phone noStatus navActive="home" go={goTo}>
    ...
    <div style={{padding:"13px 13px",border:`1.5px solid ${G[200]}`,borderRadius:"11px",background:G[50],color:G[400],fontSize:"13px",display:"flex",gap:"8px"}}>{Icon.search()}<span>가게 이름, 메뉴를 검색해보세요</span></div>
    </TopContent>
    <div onClick={()=>goTo("ai-recommend")} style={{margin:"10px 14px 0",padding:"12px 14px",borderRadius:"13px",background:`linear-gradient(120deg,${PRIMARY},#9C6FFF)`,cursor:"pointer",display:"flex",alignItems:"center",gap:"10px"}}>
    ...
    <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:"12px"}}>
      <div className="no-scrollbar" style={{display:"flex",gap:"7px",overflowX:"auto",paddingBottom:"4px",marginLeft:"-14px",paddingLeft:"14px"}}>
        {cats.map(c=><Chip key={c} label={c} active={cat===c} onClick={()=>setCat(c)}/>)}
      </div>
      <Section title="🔥 인기 가게">
        <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
          {stores.map((s,i)=>(
            <div key={i} onClick={()=>goTo("store")} style={{border:`1.5px solid ${G[200]}`,borderRadius:"13px",overflow:"hidden",cursor:"pointer",background:"#fff"}}>
