        ...
        <div style={{padding:"20px 16px 16px",background:`linear-gradient(160deg,#FFF3F0,#fff)`,borderBottom:`1px solid ${G[100]}`}}>
          <div style={{display:"flex",alignItems:"center",gap:"14px"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{marginTop:"5px"}}><Badge bg={PRIMARY} color="#fff">일반 회원</Badge></div>
            </div>
          ...
          <div style={{position:"fixed",top:0,left:0,width:"100%",height:"100%",background:"rgba(0,0,0,0.45)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{width:"340px",background:"#fff",borderRadius:"16px",padding:"20px",display:"flex",flexDirection:"column",gap:"13px",boxShadow:"0 20px 60px rgba(0,0,0,0.25)"}}>
              ...
              <div style={{display:"flex",gap:"8px"}}>
                <button onClick={()=>setEditProfile(false)} style={{flex:1,padding:"12px",borderRadius:"10px",border:`1.5px solid ${G[300]}`,background:"#fff",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",color:G[700]}}>취소</button>
                <button onClick={()=>{setProfile({...draft});setEditProfile(false);}} style={{flex:1,padding:"12px",borderRadius:"10px",border:"none",background:PRIMARY,color:"#fff",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>저장</button>
              </div>
            </div>
          </div>
        )}
              ...
              <div style={{background:"#fff",border:`1.5px solid ${G[200]}`,borderRadius:"12px",overflow:"hidden"}}>
                {grp.items.map((item,ii) => (
                  <div key={ii} onClick={()=>item.go&&go(item.go)} style={{display:"flex",alignItems:"center",gap:"12px",padding:"13px 14px",borderBottom:ii<grp.items.length-1?`1px solid ${G[100]}`:"none",cursor:item.go||item.toggle?"pointer":"default"}}>
                    ...
                    {item.toggle
                      ? <div onClick={e=>{e.stopPropagation();item.set(v=>!v);}} style={{width:"40px",height:"22px",borderRadius:"11px",background:item.val?PRIMARY:G[300],position:"relative",cursor:"pointer",flexShrink:0}}><div style={{width:"18px",height:"18px",borderRadius:"50%",background:"#fff",position:"absolute",top:"2px",left:item.val?"20px":"2px",transition:"left .2s",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/></div>
