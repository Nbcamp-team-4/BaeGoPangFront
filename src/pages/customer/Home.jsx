      ...
      <div style={{padding:"14px 14px 8px",background:"#fff",borderBottom:`1px solid ${G[100]}`,flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"9px"}}>
          <div>
            ...
            <div style={{fontSize:"15px",fontWeight:800,color:G[900],display:"flex",alignItems:"center",gap:"4px"}}>📍 광화문 1가 <span style={{fontSize:"11px",color:PRIMARY}}>▾</span></div>
          </div>
        </div>
        <div style={{padding:"9px 13px",border:`1.5px solid ${G[200]}`,borderRadius:"11px",background:G[50],color:G[400],fontSize:"13px",display:"flex",gap:"8px"}}>
          {Icon.search()}<span>가게 이름, 메뉴를 검색해보세요</span>
        </div>
      </div>
      {/* AI 추천 배너 */}
              ...
              <div key={i} onClick={()=>go("store")} style={{border:`1.5px solid ${G[200]}`,borderRadius:"13px",overflow:"hidden",cursor:"pointer",background:"#fff"}}>
                ...
                <div style={{padding:"10px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    ...
                    <Badge color="#fff" bg={PRIMARY}>{s.cat}</Badge>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:"4px",margin:"3px 0"}}>
                    <Stars v={s.rating}/><span style={{fontSize:"12px",fontWeight:700}}>{s.rating}</span><span style={{fontSize:"11px",color:G[400]}}>{s.reviews}개</span>
                  </div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:"5px",fontSize:"11px",color:G[500]}}>
                    <span>📍{s.dist}</span><span>·</span><span>최소 {s.minOrder}원</span><span>·</span><span>배달비 {s.fee}원</span><span>·</span><span>⏱{s.speed}</span>
                  </div>
                </div>
              </div>
            ))}
