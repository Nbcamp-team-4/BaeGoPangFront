      ...
      <div style={{ flex: 1, overflowY: "auto", background: "#F8F9FA" }}>
        ...
        {step === 0 && (
          <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "20px" }}>
            <div style={{ padding: "24px 16px", borderRadius: "16px", background: `linear-gradient(135deg, ${AI_COLOR}, #9C6FFF)`, textAlign: "center", color: "#fff", boxShadow: "0 4px 12px rgba(110, 64, 255, 0.2)" }}>
              <div style={{ fontSize: "32px", marginBottom: "8px" }}>🤖</div>
              <div style={{ fontSize: "16px", fontWeight: 800 }}>무엇을 먹을지 결정해 드릴까요?</div>
              <div style={{ fontSize: "12px", opacity: 0.9, marginTop: "4px" }}>3가지 질문에 답하면 AI가 맛집을 찾아드려요.</div>
            </div>

            {/* Q1. 기분/메뉴 */}
            <div>
              <div style={{ fontSize: "14px", fontWeight: 800, color: G[800], marginBottom: "10px", display: "flex", alignItems: "center", gap: "6px" }}>
                <span style={{ color: AI_COLOR }}>Q1.</span> 오늘 어떤 음식이 땡기나요?
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                {moods.map(m => (
                  <div key={m} onClick={() => setSel(s => ({ ...s, mood: m }))} style={{ 
                    padding: "14px 10px", 
                    border: `1.5px solid ${sel.mood === m ? AI_COLOR : "#fff"}`, 
                    borderRadius: "12px", 
                    background: sel.mood === m ? AI_LIGHT : "#fff", 
                    cursor: "pointer", fontSize: "12px", fontWeight: 600, color: sel.mood === m ? AI_COLOR : G[700], 
                    textAlign: "center", boxShadow: "0 2px 4px rgba(0,0,0,0.03)"
                  }}>{m}</div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                {people.map(p => (
                  <div key={p} onClick={() => setSel(s => ({ ...s, people: p }))} style={{ 
                    flex: 1, padding: "14px 0", 
                    border: `1.5px solid ${sel.people === p ? AI_COLOR : "#fff"}`, 
                    borderRadius: "12px", 
                    background: sel.people === p ? AI_LIGHT : "#fff", 
                    cursor: "pointer", fontSize: "12px", fontWeight: 600, color: sel.people === p ? AI_COLOR : G[700], 
                    textAlign: "center", boxShadow: "0 2px 4px rgba(0,0,0,0.03)"
                  }}>{p}</div>
                ))}
              </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                {budget.map(b => (
                  <div key={b} onClick={() => setSel(s => ({ ...s, budget: b }))} style={{ 
                    flex: 1, padding: "14px 0", 
                    border: `1.5px solid ${sel.budget === b ? AI_COLOR : "#fff"}`, 
                    borderRadius: "12px", 
                    background: sel.budget === b ? AI_LIGHT : "#fff", 
                    cursor: "pointer", fontSize: "11px", fontWeight: 600, color: sel.budget === b ? AI_COLOR : G[700], 
                    textAlign: "center", boxShadow: "0 2px 4px rgba(0,0,0,0.03)"
                  }}>{b}</div>
                ))}
              </div>
            ...
            <Btn variant="ai" full onClick={handleAIRequest} style={{ padding: "16px", marginTop: "10px" }}>✨ AI 추천 받기</Btn>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "12px" }}>
             ...
             <div style={{ padding: "12px 14px", borderRadius: "13px", background: `linear-gradient(120deg,${AI_COLOR},#9C6FFF)`, color: "#fff", fontWeight: 800 }}>✨ AI의 강력 추천 메뉴</div>
             {results.map((r, i) => (
                <div key={i} style={{ border: `1.5px solid ${i === 0 ? AI_COLOR : G[200]}`, borderRadius: "16px", padding: "16px", background: "#fff", boxShadow: "0 4px 6px rgba(0,0,0,0.02)" }}>
                   <div style={{ display: "flex", gap: "12px" }}>
                      ...
                      <div style={{ flex: 1 }}>
                         ...
                         <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px" }}>
                            ...
                            <Badge bg={AI_COLOR} color="#fff">{r.matchRate}% 일치</Badge>
                         </div>
                      </div>
                   </div>
                   <div style={{ marginTop: "12px", padding: "10px", background: G[50], borderRadius: "8px", fontSize: "12px", color: G[600], lineHeight: 1.5 }}>
                      💬 {r.description}
                   </div>
                </div>
             ))}
             <Btn full onClick={() => setStep(0)} style={{ marginTop: "12px" }}>↩ 다른 메뉴 추천받기</Btn>
          </div>
        )}
      </div>
    </Phone>
