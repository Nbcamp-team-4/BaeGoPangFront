...
export default function Order({ go }) {
  ...
  const fmtCardNum = (val,idx) => { const d=val.replace(/\D/g,"").slice(0,4); const n=[...cardNum]; n[idx]=d; setCardNum(n); };
  const fmtExpiry  = val => { const d=val.replace(/\D/g,"").slice(0,4); setExpiry(d.length>=3?d.slice(0,2)+"/"+d.slice(2):d); };
  ...
  return (
    <Phone navActive="cart" go={go}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",gap:"12px"}}>
        ...
        <Section title="결제 수단">
          <div style={{display:"flex",flexDirection:"column",gap:"7px"}}>
            {[{v:"card",icon:"💳",label:"신용 / 체크카드"},{v:"kakao",icon:"💛",label:"카카오페이"},{v:"toss",icon:"🔵",label:"토스"},{v:"bank",icon:"🏦",label:"무통장 입금"}].map(m => (
              <div key={m.v}>
                <Radio checked={pay===m.v} onClick={()=>setPay(m.v)}>
                  <div style={{display:"flex",alignItems:"center",gap:"9px"}}><span style={{fontSize:"18px"}}>{m.icon}</span><span style={{fontSize:"13px",fontWeight:pay===m.v?700:500,color:pay===m.v?PRIMARY:G[800]}}>{m.label}</span></div>
                </Radio>
                {m.v==="card" && pay==="card" && (
                  <div style={{marginTop:"7px",padding:"13px",border:`1.5px solid ${G[300]}`,borderRadius:"11px",background:"#fff",display:"flex",flexDirection:"column",gap:"10px"}}>
                    <div>
                      ...
                      <select value={cardType} onChange={e=>setCardType(e.target.value)} style={{width:"100%",padding:"9px 12px",border:`1.5px solid ${G[300]}`,borderRadius:"8px",fontSize:"13px",color:cardType?G[900]:G[400],background:"#fff",outline:"none",fontFamily:"inherit",appearance:"none"}}>
                    ...
                    <div>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:"4px"}}>
                        {cardNum.map((v,i) => <input key={i} value={v} onChange={e=>fmtCardNum(e.target.value,i)} maxLength={4} placeholder="0000" style={{flex:1,minWidth:0,padding:"6px 0",textAlign:"center",border:"none",borderBottom:`1.5px solid ${G[300]}`,fontSize:"13px",fontWeight:700,fontFamily:"monospace",background:"transparent",outline:"none",color:G[900],letterSpacing:"2px"}}/>)}
                      </div>
                    </div>
                    <div style={{display:"flex",gap:"9px"}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:"11px",fontWeight:700,color:G[600],marginBottom:"5px"}}>비밀번호 앞 2자리</div>
                        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                          <input value={cardPw} onChange={e=>setCardPw(e.target.value.replace(/\D/g,"").slice(0,2))} maxLength={2} type="password" placeholder="__" style={{width:"44px",padding:"6px 0",textAlign:"center",border:"none",borderBottom:`1.5px solid ${G[300]}`,fontSize:"15px",fontFamily:"monospace",background:"transparent",outline:"none",fontWeight:700}}/>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <input value={cvc} onChange={e=>setCvc(e.target.value.replace(/\D/g,"").slice(0,3))} maxLength={3} type="password" placeholder="000" style={{width:"100%",padding:"6px 0",textAlign:"center",border:"none",borderBottom:`1.5px solid ${G[300]}`,fontSize:"14px",fontFamily:"monospace",background:"transparent",outline:"none",boxSizing:"border-box",fontWeight:700}}/>
                      </div>
                    </div>
                    <div>
                      <div style={{fontSize:"11px",fontWeight:700,color:G[600],marginBottom:"5px"}}>유효기간 (YY/MM)</div>
                      <input value={expiry} onChange={e=>fmtExpiry(e.target.value)} maxLength={5} placeholder="YY/MM" style={{width:"100px",padding:"6px 0",border:"none",borderBottom:`1.5px solid ${G[300]}`,fontSize:"14px",fontFamily:"monospace",background:"transparent",outline:"none",fontWeight:700,color:G[900]}}/>
                    </div>
                  </div>
                )}
              </div>
