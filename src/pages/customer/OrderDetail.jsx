...
export default function OrderDetail({ go }) {
  ...
  const submittedReview = getSubmittedReviewByOrderId("ORD-001");
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",gap:"12px",fontSize:"12px"}}>
        ...
        <div style={{display:"flex",gap:"7px"}}>
          {!refundDone && <>
            ...
            {!submittedReview && <Btn variant="primary" style={{flex:1}} onClick={()=>navigate(`/customer/review?orderId=${encodeURIComponent('ORD-001')}`)}>리뷰 작성</Btn>}
          </>}
        ...
        {submittedReview && (
          <div style={{padding:"11px",background:"#E8F5E9",borderRadius:"10px",border:"1px solid #A5D6A7",display:"flex",flexDirection:"column",gap:"5px"}}>
            <div style={{fontSize:"12px",fontWeight:800,color:"#2E7D32"}}>리뷰 작성 완료</div>
            <div style={{fontSize:"11px",color:G[700]}}>별점 {submittedReview.rating}/5</div>
            <div style={{fontSize:"12px",color:G[800],lineHeight:"1.6"}}>{submittedReview.content}</div>
          </div>
        )}
>>>>>>> 221c319 (feat: review frontend)
      </div>
