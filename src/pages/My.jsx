...
import { G, PRIMARY, PRIMARY_LIGHT, FlatIcons, Badge, Phone, TopBar, Icon , TopContent} from "../components/UI";
...
function My({go}){
  ...
  return <Phone navActive="my" go={goTo}>
     <TopContent>
        <TopBar title="마이페이지" go={goTo} backTo="home"/>
    </TopContent>
          ...
          <div key={gi}>
            ...
            <div style={{background:"#fff",border:`1.5px solid ${G[200]}`,borderRadius:"12px",overflow:"hidden"}}>
              {grp.items.map((item,ii)=>(
                <div key={ii} onClick={()=>item.go&&goTo(item.go)} style={{display:"flex",alignItems:"center",gap:"12px",padding:"13px 14px",borderBottom:ii<grp.items.length-1?`1px solid ${G[100]}`:"none",cursor:item.go||item.toggle?"pointer":"default"}}>
